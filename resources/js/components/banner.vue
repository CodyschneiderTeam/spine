<template>
    <div v-if="display"
        ...
        <!-- Container -->
        <div :class="[colors[color], `ui-${color}`]"
             class="ui-container bg-gradient-to-br flex flex-col rounded-lg relative overflow-hidden p-8">
...
<script>
    export default
    {
        ...
        created()
        {
            this.display = ! window.localStorage.getItem(`ui_hide_banner_${this.id}`);
        },
        ...
        methods :
        {
            ...
            hide()
            {
                ...
                window.localStorage.setItem(`ui_hide_banner_${this.id}`, 1);
            }
        },
    }
</script>